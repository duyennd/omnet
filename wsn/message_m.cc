...
void Message::copy(const Message& other)
{
    ...
    delete [] this->pass;
    this->pass = (other.pass_arraysize==0) ? nullptr : new int[other.pass_arraysize];
    pass_arraysize = other.pass_arraysize;
    for (unsigned int i=0; i<pass_arraysize; i++)
        this->pass[i] = other.pass[i];
}
...
void Message::parsimPack(omnetpp::cCommBuffer *b) const
{
    ...
    b->pack(pass_arraysize);
    doParsimArrayPacking(b,this->pass,pass_arraysize);
}
...
void Message::parsimUnpack(omnetpp::cCommBuffer *b)
{
    ...
    delete [] this->pass;
    b->unpack(pass_arraysize);
    if (pass_arraysize==0) {
        this->pass = 0;
    } else {
        this->pass = new int[pass_arraysize];
        doParsimArrayUnpacking(b,this->pass,pass_arraysize);
    }
}
...
void Message::setPassArraySize(unsigned int size)
{
    int *pass2 = (size==0) ? nullptr : new int[size];
    unsigned int sz = pass_arraysize < size ? pass_arraysize : size;
...
unsigned int Message::getPassArraySize() const
{
    return pass_arraysize;
}

int Message::getPass(unsigned int k) const
{
    if (k>=pass_arraysize) throw omnetpp::cRuntimeError("Array of size %d indexed by %d", pass_arraysize, k);
    return this->pass[k];
}

void Message::setPass(unsigned int k, int pass)
{
    if (k>=pass_arraysize) throw omnetpp::cRuntimeError("Array of size %d indexed by %d", pass_arraysize, k);
    this->pass[k] = pass;
}
...
int MessageDescriptor::findField(const char *fieldName) const
{
    ...
    if (fieldName[0]=='p' && strcmp(fieldName, "pass")==0) return base+3;
...
int MessageDescriptor::getFieldArraySize(void *object, int field) const
{
    ...
    switch (field) {
        case 3: return pp->getPassArraySize();
        default: return 0;
    }
}
...
std::string MessageDescriptor::getFieldValueAsString(void *object, int field, int i) const
{
    ...
    switch (field) {
        ...
        case 3: return long2string(pp->getPass(i));
        default: return "";
    }
}
...
bool MessageDescriptor::setFieldValueAsString(void *object, int field, int i, const char *value) const
{
    ...
    switch (field) {
        ...
        case 3: pp->setPass(i,string2long(value)); return true;
