...
void Nodes::finish(){
    if(type == 3){
        ...
        for (int i = 0; i < arrayLength; i++)
            preArr[i] = 0;
        fileInput.open(("F:/omnetpp-5.6.2-src-windows/omnetpp-5.6.2/project3/Lab06output.txt"), ios::in);
        if(fileInput){
            int i = 0;
        ...
        fileInput.close();
        fileOutput.open(("F:/omnetpp-5.6.2-src-windows/omnetpp-5.6.2/project3/Lab06/output.txt"), ios::out);
        for (int i = 0; i < arrayLength; i++) {
...
//            FILE *gnplt = popen("C:\Program Files\gnuplot\bin\gnuplot","w");
//
...
//             fprintf(gnplt,"plot \"F:/omnetpp-5.6.2-src-windows/omnetpp-5.6.2/project3/Lab06/output.txt\"  matrix u ($1*100):($0*100/16) every 1::1 with lines ls 2  \n");
//             fflush(gnplt);
}
